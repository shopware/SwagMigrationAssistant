...
abstract class ShopwareConverterTest extends TestCase
{
    ...
    public function testConvert(string $fixtureFolderPath): void
    {
        ...
        if ($output !== null) {
            static::assertNotNull(
                $convertResult->getMappingUuid(),
                $this->getAssertMessage($fixtureName . ': No mappingUuid in converted result struct.')
            );
        }
     ...
     */
    abstract protected function createConverter(MappingServiceInterface $mappingService, LoggingServiceInterface $loggingService, MediaFileServiceInterface $mediaFileService, ?array $mappingArray = []): ConverterInterface;
    ...
    {
        return Shopware6MajorProfile::PROFILE_NAME;
    }
